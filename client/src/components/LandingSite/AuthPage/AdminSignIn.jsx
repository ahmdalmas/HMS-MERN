...
export default function AdminSignIn() {
  ...
  const getHostel = async () => {
    ...
    try {
      const res = await fetch("http://localhost:3001/api/admin/get-hostel", {
        method: "POST",
        headers: {
    ...
    try {
      ...
      if (result.success) {
        if (result.data.user.isAdmin) {
          ...
          // Fetch admin data and handle it
          let adminResponse = await fetch("http://localhost:3001/api/admin/get-admin", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          console.log('adminResult = ', JSON.stringify(adminResult, null, 2));
          ...
          toast.error(
            'Not an Admin, authorization denied', {
            position: "top-right",
            autoClose: 3000,
            hideProgressBar: false,
            closeOnClick: true,
