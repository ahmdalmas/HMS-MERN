...
const List = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3001/api/invoice/student", {
      method: "POST",
      headers: {
      ...
      <div className="flow-root">
        <ul role="list" className="divide-y divide-gray-700">
          {invoiceList.map((invoice) => (
            <li className="py-3 sm:py-4" key={invoice.title + invoice.date}>
...
function Home() {
  ...
  const getAttendance = async () => {
    let student = JSON.parse(localStorage.getItem("student"));
    const res = await fetch("http://localhost:3001/api/attendance/get", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
