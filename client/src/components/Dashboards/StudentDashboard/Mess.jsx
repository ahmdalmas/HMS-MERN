...
function Mess() {
  let requestMessOff = async (event) => {
    ...
    let response = await fetch("http://localhost:3001/api/Messoff/request", {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    if (student) {
      fetch("http://localhost:3001/api/Messoff/count", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        .then((result) => {
          if (result.success) {
            ...
            toast.error(result.errors[0].msg || 'Failed to update student details. Please try again.', {
              position: "top-right",
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
