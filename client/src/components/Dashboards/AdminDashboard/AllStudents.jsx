...
function AllStudents() {
  ...
  const getCSV = async () => {
    ...
    const res = await fetch("http://localhost:3001/api/student/csv", {
      method: "POST",
      headers: {
  ...
  const deleteStudent = async (id) => {
    const res = await fetch("http://localhost:3001/api/student/delete-student", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
              ...
              <li className="py-3 px-5 rounded sm:py-4 hover:bg-neutral-700 hover:scale-105 transition-all" key={student._id}>
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex gap-3">
                    <button className="hover:underline hover:text-green-600 hover:scale-125 transition-all" onClick={() => navigate(`/admin-dashboard/students/edit/${student._id}`)}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
