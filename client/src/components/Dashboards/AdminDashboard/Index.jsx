...
export default function Index() {
  const { admin, requests, addRequest, setAdmin } = useStore();
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:3001/api/request/getall', {
      method: "GET",
      headers: {
      ...
      .then((data) => {
        ...
        data.forEach((item) => addRequest(item.cms_id));
        console.log('state index: ', requests);
      })
      .catch((error) => {
  ...
  return (
    <div className="flex">
      ...
      <div className="w-full relative">
        <Topbar name={admin?.name} requests={requests} noOfRequests={requests.length}/>
