...
function Topbar({ name, requests=[], noOfRequests }) {
  ...
  const deleteRequest = async (cms_id) => {
    try {
      const res = await fetch('http://localhost:3001/api/request/', {
        ...
        body: JSON.stringify({ cms_id }),
      });
      if (res.ok) {
        removeRequest(cms_id);
      }
  ...
  return (
    <div className="py-5 px-5 flex items-center justify-between text-white w-full bg-stone-950 shadow-lg absolute top-0">
      ...
      <div className="flex gap-3">
        ...
        <div className="relative group cursor-pointer">
          ...
          {noOfRequests > 0 && (
            <span className="absolute right-0 bg-red-500 text-white font-bold rounded-full Request-badge flex items-center justify-center">
              {noOfRequests}
            </span>
          )}
              ...
              {noOfRequests === 0 ?
                ...
                : requests.map((noti) => (
                  ...
                  >
                    ...
                    <span className="group/edit relative flex" onClick={() => deleteRequest(noti)}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
