...
const registerAdmin = async (req, res) => {
    try {
        ...
        try {
            ...
            let ccnic = await Admin.findOne({ cnic });
            if (ccnic) {
                return res.status(400).json({ success, errors: [{ msg: 'Admin with provided cnic already exists' }] });
            }
            ...
            await user.save();
            if (!user) {
                return res.status(500).json({ success, errors: [{ msg: 'Failed to create user' }] });
            }
            ...
            console.log(error);
            res.status(500).send('Server2 error');
        }
    } catch (err) {
        console.log(err);
        res.status(500).json({ success, errors: [{ msg: 'Server2a error' }] });
    }
}
...
const getAdmin = async (req, res) => {
    ...
    try {
        ...
        res.status(500).send('Server3 error');
    }
}
