...
const getStudent = async (req, res) => {
    ...
    try {
        ...
        if (isAdmin) {
            return res.status(401).json({ success, errors: [{ msg: 'It looks like you are an admin. Click here to login' }] });
        }
...
const getStudentById = async (req, res) => {
    ...
    try {
        ...
        if (!id) {
            ...
            const studentData = await Student.findById(id).select('-password -user').populate('hostel');
